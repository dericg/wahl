...
For a pull request linked to a private fix thought, the signed-in owner can open **View progress → Check merge readiness**. Wahl checks GitHub live: the PR must be open, non-draft, within `dericg/wahl`, targeting and up to date with `main`, with a clean merge status and successful validation for its exact head commit. Validation is either the normal **Validate Wahl** pull-request run or the trusted `wahl/revision-validation` status created after a comment-driven revision passes tests and build. The owner must review the linked issue, repository instructions, current main, and test-site interactions before checking the review confirmation and choosing **Merge PR**. The Edge Function repeats its read-only checks and uses the existing Actions-authorized token to dispatch `merge-reviewed-pr.yml` on `main` with the approved head and base commits. That trusted workflow repeats every check and performs the squash merge with GitHub's short-lived repository token. Changed commits or main require a fresh review; errors clear the confirmation. GitHub remains authoritative for branch protections and merge permission. Public activity cards stay read-only, and issue-only PRs continue to be reviewed on GitHub. Merging does not publish a production release.

No additional GitHub or Supabase secret is required. The Edge Function reuses the existing server-only `WAHL_GITHUB_TOKEN` only to read status and invoke a workflow; it never receives merge permission. The workflow uses its ephemeral `github.token` with `Actions: read`, `Contents: write`, and `Pull requests: write`, has a five-minute timeout, serializes changes to `main`, and receives no repository secrets. Keep branch protection requiring current validation so GitHub enforces changes to main that race the final request. Enable squash merging in the repository. The workflow file must be present on `main` before GitHub can dispatch it, so PR #36 itself must be merged on GitHub; the in-site button becomes operational for subsequent eligible pull requests. Before accepting this change, manually verify readiness, confirmation, sign-out behavior, and narrow/keyboard layout; automated validation does not start a browser or perform a real merge.
...
To revise an existing automation pull request, leave an owner-authored pull-request comment in this exact form:

```text
/codex revise

Describe the requested changes here.
```

`Revise a Wahl pull request` sends the comment to Codex using the existing `OPENAI_API_KEY`, but only when the comment belongs to repository owner `dericg`, the pull request is open against `main`, and its same-repository branch starts with `codex/wahl-fix-`. Codex edits the existing branch; it does not create another issue or pull request. Trusted workflow steps enforce protected-file boundaries, run `npm test` and `npm run build`, push the exact revision, attach `wahl/revision-validation`, and request a new test deployment. Review comments without the exact `/codex revise` first line do not run automation. The workflow never merges the pull request.
