...
For a pull request linked to a private fix thought, the signed-in owner can open **View progress → Check merge readiness**. Wahl checks GitHub live: the PR must be open, non-draft, within `dericg/wahl`, targeting and up to date with `main`, with a clean merge status and a successful **Validate Wahl** pull-request run for its head commit. The owner must review the linked issue, repository instructions, current main, and test-site interactions before checking the review confirmation and choosing **Merge PR**. The Edge Function repeats its read-only checks and dispatches `merge-reviewed-pr.yml` with the approved head and base commits. That trusted workflow on `main` repeats every check and performs the squash merge with GitHub's short-lived repository token. Changed commits or main require a fresh review; errors clear the confirmation. GitHub remains authoritative for branch protections and merge permission. Public activity cards stay read-only, and issue-only PRs continue to be reviewed on GitHub. Merging does not publish a production release.

No additional GitHub or Supabase secret is required. The Edge Function reuses the existing server-only `WAHL_GITHUB_TOKEN` only to read status and request the trusted workflow; it never receives merge permission. The workflow uses its ephemeral `github.token` with `Actions: read`, `Contents: write`, and `Pull requests: write`, has a five-minute timeout, serializes changes to `main`, and receives no repository secrets. Keep branch protection requiring current validation so GitHub enforces changes to main that race the final request. Enable squash merging in the repository. The workflow file must be present on `main` before GitHub will handle its repository-dispatch event, so the merge button becomes operational for pull requests created after this feature itself is merged. Before accepting this change, manually verify readiness, confirmation, sign-out behavior, and narrow/keyboard layout; automated validation does not start a browser or perform a real merge.
