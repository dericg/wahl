...
- A test/review deployment may keep the prior semantic version while behavior is being evaluated. Once a reviewed bug fix is accepted for release, prepare the next patch version in `package.json` and the root `package-lock.json`; do not treat a deployment number or commit hash as the product version.
- The minimal PWA shell is a user-visible capability and therefore requires a pre-1.0 minor release. Its release candidate is `0.5.0` after the current `0.4.0` feature batch. The service worker may cache only the static app shell and versioned frontend assets; it must never cache Supabase responses, authentication, private thoughts, conversations, Facebook archive files, resolved media, or mutation requests.
